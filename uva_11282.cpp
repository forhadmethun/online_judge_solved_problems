...
ll dearrange_ment(ll n)
{
    if(n==1) return 0;
    if(n==2) return 1;
    if(dp[n]==0) dp[n] = (n-1)*(dearrange_ment(n-1) + dearrange_ment(n-2));
    return dp[n];
}
...
int main(int argc, char const *argv[])
{
	...
	while(scanf("%lld %lld",&n,&m)!=EOF)
    {
        ...
        ll result = 0;
        for(ll i = dif;i <=n;i++){
            result += nCr(n,i) * dearrange_ment(i);
        }
        //cout << nCr(3,3) <<endl;
        //cout << dearrange_ment(n) << endl;
        cout << result << endl;
